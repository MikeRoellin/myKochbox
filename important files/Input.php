...
<div class="header">
	<!-- Header --><!-- Header --><!-- Header --><!-- Header --><!-- Header -->
		<nav class="navbar navbar-expand-md bg-dark navbar-dark fixed-top">
			<a class="navbar-brand" href="http://localhost:8080/myKochbox/Home2.html">myKochbox</a>
			...
			<div class="collapse navbar-collapse" id="collapsibleNavbar">
				<ul class="navbar-nav nav-pills">
					<li class="nav-item">
						<a class="nav-link" href="http://localhost:8080/myKochbox/Home2.html">Startseite</a>
					</li>
					<li class="nav-item">
...
<form action="" method="post">
	...
	<p></p>
	<input class="btn btn-primary green-background" type="Submit" name="SubmitButton" value="Finde Rezept"/>
	<p></p>
</form>
...
<?php if(isset($input)){ //Ausgabe nach Drücken des Buttons
	...
	//Abfrage von Menge A(INPUT) in Menge B(Rezept) vereint mit Abfrage nach Schnittmenge A und B als auch B in Menge A
	...
	// UNION SELECT id, titel,bild, zutaten, beschreibung FROM rezept WHERE id IN
	// (SELECT id FROM normalform WHERE zutat IN ('$auswahltext'))";
	$AnzahlResultate ="SELECT COUNT(id) AS AnzahlResultate FROM rezept WHERE id IN
	(SELECT id FROM normalform WHERE zutat IN(SELECT zutat FROM auswahl)
	GROUP BY id HAVING COUNT(DISTINCT zutat) = (SELECT COUNT(zutat) FROM auswahl)
	UNION SELECT id FROM normalform WHERE zutat IN (select zutat from auswahl) GROUP BY id HAVING COUNT(DISTINCT zutat)>=2)";
...
// select group_concat(zutaten separator ', ') from zutaten where zutaten.rezept_id = 1
// SELECT zutaten FROM (SELECT id, GROUP_CONCAT(zutaten) as zutaten
// FROM zutaten
// GROUP BY rezept_id) as Ausgabe Where id = 1

	// $fehlendeZutaten = "SELECT GROUP_CONCAT(zutat) as zutaten FROM normalform JOIN matching ON matching.zutat_id=zutaten.id WHERE zutaten.rezept_id=1 and zutaten not in (SELECT zutat FROM auswahl) ";
	// $resultat2 = mysqli_query($db_link, $fehlendeZutaten)
	...
	echo "<div>";
	  echo "<table width='100%' id='my-table-id' class='table table-bordered table-responsive-sm' border=1>";
		echo "<thead class='thead-light'><tr> <th> </th><th> Name: </th> <th>Zutaten: </th><th></th></tr></thead>";
	    while($row = mysqli_fetch_assoc($resultat1)) {
	        echo "<tr>";
					echo "<td> <img class='RezeptBild' src=".$row['bild']; echo "></td>";
	        echo "<td>"; echo $row["titel"]; echo "</td>";
	        echo "<td>"; echo $row['zutaten'];
					// echo "<td>"; echo "Beschreibung:" .$row['beschreibung'];
...
//FALLS KEINE ERGEBNISSE GEFUNDEN WERDEN
	if (mysqli_num_rows($resultat1) == 0) {
	  echo "Keine exakten Treffer<br>";
		...
		(SELECT id FROM normalform WHERE zutat IN(SELECT zutat FROM auswahl)
		GROUP BY id HAVING COUNT(DISTINCT zutat) = (SELECT COUNT(zutat) FROM auswahl)
		UNION SELECT id FROM normalform WHERE zutat IN (select zutat from auswahl)) AND id not in(
			SELECT id FROM normalform WHERE zutat IN(SELECT zutat FROM auswahl)
			GROUP BY id HAVING COUNT(DISTINCT zutat) = (SELECT COUNT(zutat) FROM auswahl)
			UNION SELECT id FROM normalform WHERE zutat IN (select zutat from auswahl) GROUP BY id HAVING COUNT(DISTINCT zutat)>=2
		)";
		...
		$resultat2 = mysqli_query($db_link, $abfrage2)
			...
			if (mysqli_num_rows($resultat1) == 0 && mysqli_num_rows($resultat2) == 0) {
				echo "Leider gibt es keine Rezepte <br> Borgen sie sich eine Zutat vom Nachbar";}
